import { p as Dt, M as Ut, c as zt, q as $t, t as Wt, A as q, u as w, v as I, x as Z, y as vt, z as Nt, B as Kt, C as Xt, D as Q, E as W, n as qt, s as st, F as dt, G as wt, H as ut, I as pt, d as it, J as Yt, K as Zt } from "./entry-HwOxiB3l.js";
import { ensureViewerMetadataAsset as tt } from "./genInfo-q_XVASfE.js";
...
function ct(t, e = null, { kind: n = "" } = {}) {
  ...
  const k = () => {
    ...
    if (Number.isFinite(B) && B > 0) {
      const L = Math.max(0, Math.min(1, b / B));
      a.value = String(Math.round(L * 1e3)), S.textContent = `${lt(b)} / ${lt(B)}`;
    } else
    ...
    try {
      ...
      (L = console.debug) == null || L.call(console, j);
    }
    k();
  }, Y = (b, B) => {
    var j, V;
    ...
    if (K(b), t instanceof HTMLMediaElement) {
      try {
        ...
        (B = console.debug) == null || B.call(console, L);
      }
      R();
    }
  }, Ot = (b) => {
    ...
    var B, L, j, V;
    try {
      if ((L = (B = b == null ? void 0 : b.target) == null ? void 0 : B.closest) != null && L.call(B, "button, input, textarea, select")) return;
    ...
    if (!(!B || b != null && b.altKey || b != null && b.ctrlKey || b != null && b.metaKey)) {
      if (B === " " || B === "Spacebar") {
        (L = b.preventDefault) == null || L.call(b), mt(b);
        return;
      }
      if (B === "ArrowLeft") {
        if (!s) return;
...
function Ne() {
  ...
  function m(g = Z.getSnapshot(), { authoritative: C = !1 } = {}) {
    const E = Math.max(0, Number(g == null ? void 0 : g.queue) || 0), P = (g == null ? void 0 : g.prompt) || null, M = !!(P != null && P.currentlyExecuting), T = E > 0 || !!(P && !(P != null && P.errorDetails)), k = !!(P != null && P.errorDetails);
    C && E === 0 && !P && (c = !1, d = !1);
    const R = c || d || M || E > 0;
    if ((M || T || E > 0) && (c = !1), k) {
...
function Nn(t, e, n, o, i, s, l) {
  ...
  for (const { label: E, value: P, color: M } of a) {
    ...
    for (const D of P.split(" ")) {
      const G = x ? x + " " + D : D;
      e.measureText(G).width > k && x ? (R.push(x), x = D) : x = G;
    }
    x && R.push(x), p.push({ label: E, labelW: T, lines: R, color: M });
  }
